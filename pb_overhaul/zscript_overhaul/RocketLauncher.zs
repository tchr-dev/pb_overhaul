class PB_OverhaulRocketLauncher : PB_Weapon replaces PB_RocketLauncher
{
	default
	{
	...
	{
		super.PostBeginPlay();
		int give = PB_OverhaulRocketAmmoGive();
		if (give > 0)
		{
			AmmoGive1 = give;
		}
	...
	int PB_OverhaulRocketAmmoGive()
	{
		let p = player;
		if (p == null)
			return 20;
		let cv = Cvar.GetCvar("pb_overhaul_rocketlauncher_give", p);
		if (cv == null)
			return 20;
		int give = cv.GetInt();
		if (give < 6)
		...
		States
		{
			...
			SelectAnimation:
				TNT1 A 0 A_StartSound("weapons/sgl/inspect2", CHAN_AUTO);
			...
			CantFire:
				TNT1 A 0 A_StartSound("weapons/rocket/seeker_nofire", CHAN_AUTO);
				RL09 EE 1;
				RL98 EE 1;
				RL09 EE 1;
				RL98 EE 1;
